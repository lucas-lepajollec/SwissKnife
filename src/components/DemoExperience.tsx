...
const LINKS = {
  site: 'https://swissknife.lucas-homelab.fr',
  docs: 'https://docs.swissknife.lucas-homelab.fr',
  source: 'https://github.com/lucas-lepajollec/SwissKnife',
};
...
export function DemoExperience() {
  ...
  const resetDemo = () => {
    ...
    setAnnouncement(demo.resetting);
    window.location.reload();
  };

  const cards = [
    { title: demo.try, text: demo.tryBody },
    { title: demo.sim, text: demo.simBody },
    { title: demo.never, text: demo.neverBody },
  ];
    ...
    <>
      ...
      >
        <div className="relative overflow-hidden px-5 py-6 sm:px-8 sm:py-8">
          ...
          <nav className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-xs text-white/50" aria-label={demo.public}>
            <a className="underline decoration-white/20 underline-offset-4 hover:text-white" href={LINKS.site} target="_blank" rel="noreferrer">{demo.site}</a>
            <a className="underline decoration-white/20 underline-offset-4 hover:text-white" href={LINKS.docs} target="_blank" rel="noreferrer">{demo.docs}</a>
            <a className="underline decoration-white/20 underline-offset-4 hover:text-white" href={LINKS.source} target="_blank" rel="noreferrer">{demo.source}</a>
          </nav>
